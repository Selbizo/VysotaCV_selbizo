...
int main()
{
    ...
    cv::VideoCapture camL("http://10.142.69.71:4747/video"); //xiaomi
    cv::VideoCapture camR("http://10.142.69.133:4747/video"); //pixel

    // Check if left camera is attched
    if (!camL.isOpened())
    {
        std::cout << "Could not open right camera with index : "  << std::endl;
        return -1;
    }
    ...
    if (!camL.isOpened())
    {
        std::cout << "Could not open left camera with index : " << std::endl;
        return -1;
    }
